...
var AUWLBook = function() {
  ...
  var migrationStatus = '';
  var localeInfo = 'US';
  var uwlMigrationTreatment = 'T1';
  var migrationNoticeText = 'Add to Amazon Wish List is' + ' ' + 'retiring.';
  var retireDateText = '2016-07-31';
  var upgradeNowText = 'Install Amazon Assistant';
  window.bookmarkletRequest = false;
  window.AAInstalled  = false;
  window.uwlMigrationTreatment = uwlMigrationTreatment;
  window.migrationStatus = migrationStatus;
  window.migrationDebug = '';

  var CONSTANTS = {
    BOOKMARKLET_CLOSE_LINK        : "UWL-Migration-Close-Link-Bookmarklet",
    UWL_MESSAGE_TYPE              : "UWLMigration",
    UWL_MIGRATION_FRAME           : "UWLMigrationFrame",
    UWL_MIGRATION_HIDDEN_FRAME    : "UWLMigrationHiddenFrame",
    UWL_MIGRATION_METRICS_FRAME   : "uwlMigrationMetricsIFrame",
    UWL_DETECT_VIEW               : "detect",
    UWL_AA_DETECT_MESSAGE         : "UWL-Migration-AA-Extension-Detection",
    UWL_AA_INSTALLED_MESSAGE      : "UWL-Migration-AA-Extension-Installed",
    UWL_BKM_POPOVER_MESSAGE       : "UWL-Migration-Show-Popover-Link-Bookmarklet",
    UWL_CONFIRMATION_MESSAGE      : "UWL-Migration-Show-confirmation-view",
    UWL_CONFIRMATION_VIEW         : "confirmation",
    UWL_FF_INLINE_INSTALL_MESSAGE : "UWL-Migration-Inline-Install-Firefox",
    UWL_SF_MANUAL_INSTALL_MESSAGE : "UWL-Migration-Safari-Manual-Install",
    UWL_STATUS_CONFIRMATION       : "confirmation",
    UWL_STATUS_UPGRADED           : "upgraded",
    UWL_STATUS_NOTICE             : "notice",
    UWL_STATUS_DETECT             : "detect",
    UWL_STATUS_RETIRE             : "retired",
    UWL_STATUS_LOADER             : "loader",
    UWL_AA_UPGRADENOW_LINK        : "AAUpgradeNowLink",
    UWL_SKIP_UPGRADING_ITEM       : "SkipUpgrading",
    UWL_SKIP_UPGRADING_MESSAGE    : "UWL-Migration-Skip-Upgrading",
    UWL_AA_DETECTION_TIMEOUT      : 5,
    UWL_FF_LOADER_TIMEOUT         : 3,
    UWL_DEFAULT_HORIZONTE_EP      : "horizonte.browserapps.amazon.com"
  };

  var isUAContain = function (userAgent, keyword) {
        return (userAgent.toLowerCase().indexOf(keyword) != -1);
  };

  /*
   * For bookmarklet case, no browser info from request. We will detect browser info
   *    from userAgent.
   */
  var getBrowserType = function () {
    var navigator = window.navigator;
    if ((navigator && navigator.userAgent && isUAContain(navigator.userAgent, "trident"))
        || (navigator && navigator.userAgent && isUAContain(navigator.userAgent, "msie"))
        || (document && document.documentMode)) {
        return "ie";
    } else if (isUAContain(navigator.userAgent, "firefox")) {
        return "ff2";
    } else if (isUAContain(navigator.userAgent, "chrome")) {
        return "chr";
    } else if (isUAContain(navigator.userAgent, "safari")) {
        return "saf";
    } else {
        return "unknown";
    }
  ...
  /*
   * For request from UWL extension, it contains browser info and need convert it one supported
   *    by horizonte view.
   */
  var setWindowBrowser = function () {
    switch (browser) {
        case 'saf':
            window.currentBrowser = 'sf';
            break;
        case 'ff2':
            window.currentBrowser = 'ff';
            break;
        case 'chr':
            window.currentBrowser = 'cr';
            break;
        default  :
  ...
  /*
   * Register window message listener for bookmarklet case.
   */
  var registerBookmarkLetHandler = function () {
    window.addEventListener("message", function(message) {
        if (message.data
            && message.data.id
            && message.data.messageType
            && message.data.id === CONSTANTS.BOOKMARKLET_CLOSE_LINK
            && message.data.messageType === CONSTANTS.UWL_MESSAGE_TYPE) {
            /*
             * Hide "UWLMigrationFrame" IFrame.
             */
            Boxer.closeUWLMigrationIFrame(CONSTANTS.UWL_MIGRATION_FRAME);
            return;
        }
        ...
        if (message.data
            && message.data.id
            && message.data.messageType
            && message.data.id === CONSTANTS.UWL_AA_DETECT_MESSAGE
            && message.data.messageType === CONSTANTS.UWL_MESSAGE_TYPE) {
            /*
             * Will create hidden iframe and trigger AA extension detection.
             */
            Boxer.createUWLMigrationIFrame(CONSTANTS.UWL_DETECT_VIEW, window.bookmarkletRequest);
            Boxer.refreshUWLMigrationIFrame(CONSTANTS.UWL_DETECT_VIEW, window.bookmarkletRequest);
            return;
        }
        ...
        if (message.data
            && message.data.id
            && message.data.messageType
            && message.data.id === CONSTANTS.UWL_AA_INSTALLED_MESSAGE
            && message.data.messageType === CONSTANTS.UWL_MESSAGE_TYPE) {
            /*
             * In case AA extension installed, AADetectionAction inside hidden iframe will post message
             * to this top frame.
             */
            window.AAInstalled = true;
            if (window.migrationDebug && window.migrationDebug === 'true' && console.log) {
                console.log("AA Detection Succeed at:" + Date.now());
            }
            return;
        }
        ...
        if (message.data
            && message.data.id
            && message.data.messageType
            && message.data.id === CONSTANTS.UWL_BKM_POPOVER_MESSAGE
            && message.data.messageType === CONSTANTS.UWL_MESSAGE_TYPE) {
            /*
             * Show UWL wishlist popover view.
             */
            Boxer.showUWLPopover();
            return;
        }
        ...
        if (message.data
            && message.data.id
            && message.data.messageType
            && message.data.id === CONSTANTS.UWL_SKIP_UPGRADING_MESSAGE
            && message.data.messageType === CONSTANTS.UWL_MESSAGE_TYPE) {
            /*
             * Set skip upgrading status to local storage.
             */
            Boxer.setGurupaLocalStorage(CONSTANTS.UWL_SKIP_UPGRADING_ITEM, 'true');
            return;
        }
        ...
        if (message.data
            && message.data.id
            && message.data.messageType
            && message.data.id === CONSTANTS.UWL_CONFIRMATION_MESSAGE
            && message.data.messageType === CONSTANTS.UWL_MESSAGE_TYPE) {
            /*
             * Show migration confirmation view.
             */
            Boxer.createUWLMigrationIFrame(CONSTANTS.UWL_CONFIRMATION_VIEW, window.bookmarkletRequest);
            Boxer.refreshUWLMigrationIFrame(CONSTANTS.UWL_CONFIRMATION_VIEW, window.bookmarkletRequest);
            return;
        }
        ...
        if (message.data
            ...
            && message.data.id === CONSTANTS.UWL_FF_INLINE_INSTALL_MESSAGE
            && message.data.messageType === CONSTANTS.UWL_MESSAGE_TYPE) {
            /*
             * For Firefox inline install, redirect to the inline url and will trigger
             *  the inline installation.
             */
            Boxer.firefoxInlineInstall(message.data.actionUrl);
            return;
        }
        ...
        if (message.data
            && message.data.id
            && message.data.messageType
            && message.data.id === CONSTANTS.UWL_SF_MANUAL_INSTALL_MESSAGE
            && message.data.messageType === CONSTANTS.UWL_MESSAGE_TYPE) {
            /*
             * Safari manual install will be redirected in horizonte view; Follow same strategy,
             *  close migration view here.
             */
            Boxer.closeUWLMigrationIFrame(CONSTANTS.UWL_MIGRATION_FRAME);
            return;
        }
  ...
  var gurupaServerName = 'www.amazon.com';
  var horizonteServerName = 'horizonte.browserapps.amazon.com';
  ...
  var InitialContext = function () {
      ...
      if (!horizonteServerName || horizonteServerName === '') {
          horizonteServerName = CONSTANTS.UWL_DEFAULT_HORIZONTE_EP;
      }

      registerBookmarkLetHandler();

      /*
       * For Weblab 'C' case, turn off migration flag and switch back to original popover view.
       */
      setWindowBrowser();
      if (window.uwlMigrationTreatment === 'C'
        || window.currentBrowser === 'sf'
        || window.currentBrowser === '') {
          migrationFlag = null;
      }
...
var feedbackUrl = 'http://www.amazon.com/gp/feature.html?ie=UTF8&docId=1000807741';
var learnMoreImgSrc = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/learn-more._CB144857397_.png';
var transitionUrl = 'http://www.amazon.com/gp/BIT/theamazonapp';
...
var transparentImg    = 'https://images-na.ssl-images-amazon.com/images/G/01/misc/transparent-pixel._CB192551059_.gif';
var aaLoadingImg      = 'https://images-na.ssl-images-amazon.com/images/G/01/associates/BIT/aa/gateway/Loading-small._CB314562880_.gif';

var prevImgBtnImg     = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/btn-arrow-left2._CB200860388_.png';
var nextImgBtnImg     = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/btn-arrow-right2._CB200860388_.png';
var nextPrevBtnImg     = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/arrow-new._CB148257769_.png';
var outCloseBox       = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/btn-close-x._CB192250362_.gif';
var standardCloseBox  = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/close-new._CB149468554_.png';
var cancelBtnImg      = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/btn-cancel._CB192250363_.gif';
var addItemBtnImg     = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/Add-to-list-med._CB192250362_.gif';
var amznTinyLogo      = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/UWL-logo-by-Amzn-tiny._CB156405369_.png';
var noImageImg        = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/bkm-no-img._CB192250363_.gif';
var questionMark      = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/question-mark._CB153201332_.gif';

var outCornerUpLeft   = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/po_top_left_gray-new._CB148510300_.png'; 
var outCornerUpRight  = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/po_top_right_gray-new._CB148510300_.png';
var outCornerLowLeft  = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/po_bottom_left_gray-new._CB148510302_.png';
var outCornerLowRight = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/po_bottom_right_gray-new._CB148510303_.png';
var outEdgeUp         = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/po_top_gray-new._CB148510303_.png'; 
var outEdgeLow        = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/po_bottom._CB200122327_.png';
var outEdgeLeft       = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/po_left_gray-new._CB148510297_.png';
var outEdgeRight      = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/po_right_gray-new._CB148510303_.png';

var closeBtnImg       = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/btn-close._CB192250357_.gif';
var closeBtnImg2      = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/btn-close._CB192250357_.gif';
var goToListBtnImg    = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/btn-view-this-list._CB192250362_.gif';
var viewListBtnImg    = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/btn-view-this-list._CB192250362_.gif';
var contShoppingBtnImg = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/btn-continue-shopping._CB192250357_.gif';
var amznLogoSuccessImg = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/amazonlogo_sm-new._CB149468552_.png';

var footerLeftImg = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/uwl-bkm-lower-lt._CB192250356_.gif';
var footerRightImg = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/uwl-bkm-lower-rt._CB192250356_.gif';
var gradientImg = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/uwl-bkm-grad-2._CB192250356_.gif';
var uwlTopLogoImg = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/Amazon_Wish_List_longlogo-new._CB149468553_.png';

var alertImg = 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/common/errors-alerts/error-sprite-mini._CB192240838_.gif';
...
function shmNullPad(elem){
  ...
  function shmSetStartPos () {
    if(floater) {
      ...
      floater.style.top = (60 + scroll.Y) + "px";
    }
  }
...
var ButtonMkr = function() {
  ...
  return {
    ...
    goToList : function(suffix) {
      ...
      listLink.href = 'http://www.amazon.com/gp/wishlist';
...
var Boxer = function() {
  ...
  return {
    ...
    mkBox : function(migrationFlag, parent, body, content, width, showHeader, canMove, closeCode, closeRight, closeTop){
      ...
      var topMidEdge = Boxer.genTopMidEdge({ migrationFlag: migrationFlag, edge: 'top', image: outEdgeUp, showHeader: showHeader, contentWidth: width, closeCode: closeCode, closeRight: closeRight, closeTop: closeTop });
    ...
    genTopMidEdge : function(params) {
      ...
      if (edge == 'top' && params['showHeader']) {
        ...
        if (params['migrationFlag']) {
          shmStyleElement(contentDiv, {display: 'block', position: 'relative', width: (params['contentWidth'] + 9) + 'px', height: '43px',background: 'url(' + uwlTopLogoImg + ') no-repeat center top', backgroundColor: 'white' });
        } else {
        ...
        if (params['migrationFlag']) {
          cell.appendChild(Boxer.genUpgradeDiv(({contentWidth: params['contentWidth']})));
    ...
    /*
     * Check AA installation status within timeout(second) and refresh UWL migration iframe after the check
     */
    checkAABeforeUWLIFrame : function(timeout, status, isBookmarklet) {
        setTimeout(function() {
            if (window.pageAACounter++ < timeout && window.AAInstalled === false) {
                Boxer.checkAABeforeUWLIFrame(timeout, status, isBookmarklet);
            } else {
                if (window.AAInstalled === true) {
                    if (isBookmarklet === true) {
                        status = CONSTANTS.UWL_STATUS_CONFIRMATION;
                    } else {
                        status = CONSTANTS.UWL_STATUS_UPGRADED;
                    }
                } else {
                    status = CONSTANTS.UWL_STATUS_NOTICE;
                }
                Boxer.refreshUWLMigrationIFrame(status, isBookmarklet);
            }
        }, 1000);
    },

    /*
     * Refresh UWL migration view in iframe. It will load horizonte view base on migration status.
     */
    refreshUWLMigrationIFrame : function(status, isBookmarklet) {
        try {
            var frameID = CONSTANTS.UWL_MIGRATION_FRAME;
            if (status === CONSTANTS.UWL_STATUS_DETECT) {
                frameID = CONSTANTS.UWL_MIGRATION_HIDDEN_FRAME;
            }
            var uwlMigrationFrame = document.getElementById(frameID);

            if (!window.migrationStatus || window.migrationStatus === '') {
                window.migrationStatus = CONSTANTS.UWL_STATUS_NOTICE;
            }
            if (!status || status === '') {
                status = window.migrationStatus;
            }
            if (status !== CONSTANTS.UWL_STATUS_DETECT
                && status !== CONSTANTS.UWL_STATUS_CONFIRMATION
                && status !== CONSTANTS.UWL_STATUS_LOADER
                && window.AAInstalled === true) {
                status = CONSTANTS.UWL_STATUS_UPGRADED;
            }
            if (status === CONSTANTS.UWL_STATUS_NOTICE) {
                var retireTimeline = Date.parse(retireDateText);
                if (retireTimeline < Date.now()) {
                    status = CONSTANTS.UWL_STATUS_RETIRE;
                }
            }
            if (status === CONSTANTS.UWL_STATUS_LOADER) {
                if (window.currentBrowser === 'ff') {
                    setTimeout(function() {
                        Boxer.loaderScreen(uwlMigrationFrame);
                    }, CONSTANTS.UWL_FF_LOADER_TIMEOUT);
                } else {
                    Boxer.loaderScreen(uwlMigrationFrame);
                }
                return;
            ...
            if (status === CONSTANTS.UWL_STATUS_CONFIRMATION) {
    ...
    /*
     * Load loader screen in iframe.
     */
    loaderScreen : function(iframe) {
        try {
            var closeDiv   = shmCreateElement('div');
            var closeBoxImg = shmStyleElement(shmCreateElement('img', {border: '0', src: standardCloseBox, alt: shmEntityDecode(altClose)}, {padding: '2px 0'}));
            var closeBox = shmStyleElement(shmCreateElement('a', { href: '#', width: '17px', target: '_self' }), {position: 'absolute', right: '16px', top: '16px', marginRight: '7px'});
            Events.addHandler(closeBox, 'click', function() {Boxer.closeUWLMigrationIFrame(CONSTANTS.UWL_MIGRATION_FRAME);});
            closeBox.appendChild(closeBoxImg);
            closeDiv.appendChild(closeBox);

            var loaderDiv  = shmCreateElement('div');
            loaderDiv.style.backgroundImage = 'url("' + aaLoadingImg + '")';
            loaderDiv.style.backgroundRepeat = 'no-repeat';
            loaderDiv.style.backgroundPosition = 'center center';
            loaderDiv.style.position = 'absolute';
            loaderDiv.style.zIndex = '99999';
            loaderDiv.style.top = '0';
            loaderDiv.style.right = '0';
            loaderDiv.style.left = '0';
            loaderDiv.style.bottom = '0';
            loaderDiv.style.margin = '0';
            loaderDiv.style.backgroundColor = 'rgba(255, 255, 255, 0.83)';
            loaderDiv.style.flexDirection = 'column';
            loaderDiv.style.display = 'flex';

            iframe.contentWindow.document.body.style.backgroundColor='#fff';
            iframe.contentWindow.document.body.appendChild(closeDiv);
            iframe.contentWindow.document.body.appendChild(loaderDiv);
        } catch (error) {
            if (window.migrationDebug && window.migrationDebug === 'true' && console.log) {
                console.log(error);
            }
    ...
    /*
     * Create UWL migration iframe. In case status as 'detect', it will create hidden iframe.
     */
    createUWLMigrationIFrame : function(status, isBookmarklet) {
        try {
            var uwlMigrationFrame = document.createElement("iframe");
            if (!window.migrationStatus || window.migrationStatus === '') {
                window.migrationStatus = CONSTANTS.UWL_STATUS_NOTICE;
            }
            if (!status || status === '') {
            ...
            if (status === CONSTANTS.UWL_STATUS_DETECT) {
                ...
                var rightOffset = shmGetRightOffset();
                var scroll = shmGetScroll();
                uwlMigrationFrame.style.top = (66 + scroll.Y) + "px";
                uwlMigrationFrame.style.right = (39 + scroll.X - rightOffset) + "px";
                ...
                uwlMigrationFrame.style.zIndex = "99999";
            }
            uwlMigrationFrame.setAttribute("id", frameID);
            Boxer.closeUWLMigrationIFrame(frameID);
            document.body.appendChild(uwlMigrationFrame);
        } catch (error) {
            if (window.migrationDebug && window.migrationDebug === 'true' && console.log) {
                console.log(error);
            }
    ...
    /*
     * Close UWL migration iframe by ID
     */
    closeUWLMigrationIFrame : function(id) {
    ...
    /*
     * Firefox inline install for bookmarklet case
     */
    firefoxInlineInstall : function(inlineUrl) {
        if (window.currentBrowser === 'ff') {
    ...
    /*
     * Upgrade Now link in popover view will show Horizonte migration view:
     *  Notice view   -- Apply to both bookmarklet and UWL extension: not upgrade to AA yet
     *  Upgraded view -- Only apply to UWL extension migration: upgraded to AA while not deactivated/uninstall UWL
     *  Confirmation view -- Apply to both bookmarklet and UWL extension:
     *                              bookmarklet   -- upgrade to AA already
     *                              UWL extension -- upgrade to AA and deactivated/uninstall UWL already
     */
    upgradeNowHandler : function(status, isBookmarklet) {
        /*
         * For bookmarklet case and Firefox UWL extension, we will follow below sequence
         *  1. create migration iframe;
         *  2. load Loader screen;
         *  3. create hidden detection iframe;
         *  4. load horizonte detect view to execute the AA detection logic;
         *  5. If AA not installed, refresh migration iframe with Notice view;
         *  6. If installed, it will refresh migration iframe with either Confirmation or Upgraded view;
         */
        window.AAInstalled = false;
        Boxer.createUWLMigrationIFrame(CONSTANTS.UWL_STATUS_LOADER, isBookmarklet);
        Boxer.refreshUWLMigrationIFrame(CONSTANTS.UWL_STATUS_LOADER, isBookmarklet);
        if (window.migrationDebug && window.migrationDebug === 'true' && console.log) {
            console.log("AA Detection start from:" + Date.now());
        }
        Boxer.createUWLMigrationIFrame(CONSTANTS.UWL_STATUS_DETECT, isBookmarklet);
        Boxer.refreshUWLMigrationIFrame(CONSTANTS.UWL_STATUS_DETECT, isBookmarklet);
        window.pageAACounter = 0;

        if (window.AAInstalled === false) {
            Boxer.checkAABeforeUWLIFrame(CONSTANTS.UWL_AA_DETECTION_TIMEOUT, '', isBookmarklet);
        } else {
            Boxer.checkAABeforeUWLIFrame(CONSTANTS.UWL_AA_DETECTION_TIMEOUT, status, isBookmarklet);
        }
        Popover.hidePopover();
        return false;
    },
    ...
    /*
     * Show Popover view upgrade banner by adding mig flag in the request.
     */
    showUWLPopover : function() {
        var bookmarklet = "(function(){var w=window,l=w.location,d=w.document,s=d.createElement('script'),e=encodeURIComponent,o='object',n='AUWLBookAA__LANG__',u='https://__DOMAIN__/wishlist/add',r='readyState',T=setTimeout,a='setAttribute',g=function(){d[r]&&d[r]!='complete'?T(g,200):!w[n]?(s[a]('charset','UTF-8'),s[a]('src',u+'.js?mig=true&loc='+e(l)+'&b='+n),d.body.appendChild(s),f()):f()},f=function(){!w[n]?T(f,200):w[n].showPopover({showOriginal:'true'})};typeof s!=o?l.href=u+'?u='+e(l)+'&t='+e(d.title):g()}())".replace("__DOMAIN__",window.UWLResponseDomain).replace("__LANG__",window.UWLResponseLang);
    ...
    /*
     * Set local storage to Gurupa page.
     */
    setGurupaLocalStorage : function(itemName, itemValue) {
        try {
    ...
    /*
     * Get local storage from Gurupa page.
     */
    getGurupaLocalStorage : function(itemName) {
        var itemValue = '';
        try {
    ...
    /*
     * Get skip upgrading status from local storage.
     */
    getSkipUpgradingStatus : function(itemName) {
        var skipUpgrading = Boxer.getGurupaLocalStorage(itemName);
        var retireTimeline = Date.parse(retireDateText);
        if (retireTimeline < Date.now()) {
            skipUpgrading = 'false';
        }
        return skipUpgrading;
    },

    /**
     * Create a iframe to access AA assets where we have scripts to generate and send out metrics.
     */
    createMetricsIFrame: function() {
        var metricsIFrameID = CONSTANTS.UWL_MIGRATION_METRICS_FRAME;
        // check if it exists already and remove if it does
        var metricsIFrame = document.getElementById(metricsIFrameID);
        if (metricsIFrame && metricsIFrame.parentNode) {
            metricsIFrame.parentNode.removeChild(metricsIFrame);
        }
        // then create a new iframe
        metricsIFrame = document.createElement("iframe");
        // hide it
        metricsIFrame.style.display = "none";
        metricsIFrame.setAttribute("id", metricsIFrameID);
        // set src of iframe
        metricsIFrame.setAttribute("src", "https://" + horizonteServerName + "/uwlmigration?browser=" +
            window.currentBrowser + "&status=banner&bookmarklet=" + window.bookmarkletRequest);
        // append it to document
        document.body.appendChild(metricsIFrame);
    },

    genUpgradeDiv : function(params) {
      var upgradeDiv = shmCreateElement('div');
      var cell = shmCreateElement('span', {lineHeight: '40px'});
      cell.appendChild(stn(migrationNoticeText));
      cell.style.lineHeight = "40px";
      cell.style.color = "#000";
      cell.style.setProperty('font-size', '12px', 'important');

      var upgradeNowLink = shmCreateElement('a', {href: '#', id: CONSTANTS.UWL_AA_UPGRADENOW_LINK, fontSize: '12px', fontFamily: 'Helvetica', lineHeight: '40px'});
      upgradeNowLink.innerHTML = upgradeNowText;
      upgradeNowLink.style.lineHeight = "40px";
      upgradeNowLink.style.textDecoration = "underline";
      upgradeNowLink.style.color = "#000";
      upgradeNowLink.style.marginLeft = "5px";

      Events.addHandler(upgradeNowLink, 'click', function(e) {Boxer.setGurupaLocalStorage(CONSTANTS.UWL_SKIP_UPGRADING_ITEM, 'false');Boxer.upgradeNowHandler('', window.bookmarkletRequest)});
      cell.appendChild(upgradeNowLink);
      cell.appendChild(stn('.'));

      upgradeDiv.appendChild(cell);
      shmStyleElement(upgradeDiv, {display: 'block', position: 'relative', width: '100%', height: '40px', borderRadius: '4px 4px 0 0', marginTop: '-12px', marginBottom: '11px', backgroundColor: '#F5A623', textAlign: 'center', align: 'center'});

      // send metrics for banner
      Boxer.createMetricsIFrame();
    ...
    if ((typeof window.addEventListener != 'undefined' || typeof window.attachEvent != 'undefined') && typeof window.postMessage != 'undefined') {
      ...
      var bgSprite = 'url(https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/AddtoWL_beacon-new2._CB391079737_.png)';
...
var initSelect = function(skin){
  ...
  if (!regselect.selectedIndex) {
    var opts = regselect.options;
    var re = new RegExp(skin, 'g');
    if (opts) {
      for (var i = 0;i < opts.length;i++) {
        var name = opts[i].value;
        if (name.match(/noreg/g) && name.match(re)) {
...
var menuGen = (function() {
    ...
    return function(position) {
        if(isMenuMade == 0) {
           ...
           var nVHLstyle = {backgroundImage:'url(https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/poSprite._CB158341484_.png)',
            ...
            var nVHMstyle = {backgroundImage:'url(https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/poSprite._CB158341484_.png)',
                             backgroundAttachment:'scroll',
            ...
            var nVHRstyle = {backgroundImage:'url(https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/poSprite._CB158341484_.png)',
            ...
            var nVBLstyle = {backgroundImage:'url(https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/poSides._CB158341487_.png)',
            ...
            var nVBRstyle = {backgroundImage:'url(https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/poSides._CB158341487_.png)',
                             backgroundAttachment:'scroll',
                             backgroundRepeat:'repeat-y',
                             height:'0px',
                             position:'absolute',
            ...
            var nVFLstyle = {backgroundImage:'url(https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/poSprite._CB158341484_.png)',
            ...
            var nVFMstyle = {backgroundImage:'url(https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/poSprite._CB158341484_.png)',
                             backgroundAttachment:'scroll',
                             backgroundRepeat:'repeat-x',
                             marginLeft:shadowFW,
                             display:'block',
            ...
            var nVFRstyle = {backgroundImage:'url(https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/poSprite._CB158341484_.png)',
  ...
  Comm.listen('fixcss', function (d,e) {
    Popover.fixCSS(d.show);
  });
  ...
  var fixCSS = (function () {
    var splitButtonOnHide = '#auwlPopover iframe {' +
      'display: block !important;' +
      'visibility: visible !important;' +
      'height:32px !important;' +
      'overflow:visible !important;}';
    var splitButtonOnShow = '#auwlPopover iframe {' +
      'display: block !important;' +
      'visibility: visible !important;' +
      'height:200px !important;' +
      'overflow:visible !important;}';
    var pseudoClassRule = '#auwlPopover td:after {content: ".";display: none;}';
    var fixCSSDone = document.getElementById('auwlCSS') !== null;
    return function (show) {
       var fixCSSDone = document.getElementById('auwlCSS') !== null;
       if (!fixCSSDone) {
        var style = document.createElement('style');
        style.setAttribute('type','text/css');
        style.innerHTML = pseudoClassRule;
        document.body.appendChild(style);

        style = document.createElement('style');
        style.setAttribute('type','text/css');
        style.setAttribute('id','auwlCSS');
        style.innerHTML = splitButtonOnHide;
        document.body.appendChild(style);
      } else {
        var style = document.getElementById('auwlCSS');
        if (style) {
          if (show) {
            style.innerHTML = splitButtonOnShow;
          } else {
            style.innerHTML = splitButtonOnHide;
          }
        }
  ...
  function displayProductMatchBlock() {
    ...
    if( !isBKMSourceDomain() ) {
        ...
        var spinner = shmCreateElement('img',{id: 'auwlAsinMatchSpinner', src : 'https://images-na.ssl-images-amazon.com/images/G/01/x-locale/communities/gifting/universal/bkm/spinner._CB157256755_.gif' } , { display : 'inline' , paddingLeft : '5px'});
  ...
  extendFloater : function(response) {
    ...
    openUWLInitialView : function(status){
      if (window.uwlMigrationTreatment === 'T1'
        && (window.currentBrowser !== 'ff' || window.location.href !== 'about:newtab')
        && Boxer.getSkipUpgradingStatus(CONSTANTS.UWL_SKIP_UPGRADING_ITEM) !== 'true') {
        Boxer.upgradeNowHandler('', false);
      } else {
        Popover.showPopover({showOriginal:'true'});
      }
    },
    showPopover : function(args){
      ...
      if (migrationFlag !== null
        && (!args || (args['showOriginal'] !== 'true'))
        && Boxer.getSkipUpgradingStatus(CONSTANTS.UWL_SKIP_UPGRADING_ITEM) !== 'true') {
        /*
         * Overwrite default showPopover() call by show migration view instead; For original
         *  popover view, pass parameter {showOriginal:'true'} to achieve.
         */
        if (!args) {
            window.bookmarkletRequest = true;
        }
        Boxer.upgradeNowHandler('', window.bookmarkletRequest);
        return;
      }
      ...
      var a = '', f = arguments && arguments.callee, depth = 100;
      while (f && (depth > 0)) {
        a = f;
        f = f.caller;
        depth--;
      ...
      a = a.toString();
      if (a.match(/showpopover/i)) {
        Popover.hidePopFlag = true;
      } else {
        Popover.hidePopFlag = false;
      }
      ...
      if (!floater) {
        ...
        Events.addHandler(document, 'click', function(e) {
          ...
          if (Popover.hidePopFlag && (e.button == 0 || e.which == 1)) {
            Popover.hidePopover();
          }
          Popover.hidePopFlag = true;
        });
        ...
        Boxer.mkBox(migrationFlag, null, tbody, content, popWidth, true, true, function() { Popover.hidePopover(); return false; }, '5px', '2px');
    ...
    genVendorUpsell : function(wrapperCell) { 
        ...
        var logoImg = shmCreateElement('img', {src: "https://images-na.ssl-images-amazon.com/images/G/01/misc/untranslatable-image-id.jpg", alt: shmEntityDecode("")}, {margin: '10px 10px 4px 0', display: 'inline'} );
        logoCell.appendChild(logoImg); 
        trow.appendChild(logoCell); 

        var contentCell = shmCreateElement('td');
        contentCell.appendChild( shmCreateElement('span', {id: "vendorUpsellHeader"}, {display: 'block', marginTop: '10px', color: '#E47911', fontWeight: 'bold', fontSize: '13px'}, stn("")) );
        contentCell.appendChild( shmCreateElement('div', {id: "vendorUpsellDescription"}, {fontSize: '12px', marginTop: '5px', color: '#333333'}, stn("")) );

        var buttonDiv = shmCreateElement('div', {id: "vendorUpsellButtonDiv"}, {height: '31px', margin: '5px 0 10px', display: 'block'});
        
        var learnMoreImg = shmCreateElement('img', {src: learnMoreImgSrc, alt: shmEntityDecode("")}, {marginTop: '4px', display: 'inline'} );
        var learnMoreBtn = shmCreateElement('a', {href: "", target: '_blank'}, {cursor: 'pointer'});
        ...
        var installImg = shmCreateElement('img', {src: "https://images-na.ssl-images-amazon.com/images/G/01/misc/untranslatable-image-id.jpg", alt: shmEntityDecode("")}, {margin: '4px 0 0 8px', display: 'inline'} );
        var installBtn = shmCreateElement('a', {href: "", target: '_self'}, {cursor: 'pointer'});
    ...
    updateData : function(isAsinMatch) {
      ...
      if (isAsinMatch) {
        var myurl = 'https://www.amazon.com/wishlist/add';
      } else {
        var myurl = 'https://www.amazon.com/wishlist/add';
